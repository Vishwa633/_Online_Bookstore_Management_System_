...
public class BookQuickSort {
    ...
    // Sort by price
    public static void quickSortByPrice(List<Book> books, int low, int high) {
        if (low < high) {
            int pi = partitionByPrice(books, low, high);
            quickSortByPrice(books, low, pi - 1);
            quickSortByPrice(books, pi + 1, high);
        }
    }

    private static int partitionByPrice(List<Book> books, int low, int high) {
